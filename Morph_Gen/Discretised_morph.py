...
for l in range(a_tot):
    for m in range(b_tot):
        a_tot_count = 1
        b_tot_count = 1
        e_coeffs, w_coeffs = read_coeffs(l+1, m+1)
        ...
        'H-Output-3', rebar=EXCLUDE, region=
                                                   ...
                                                   sectionPoints=DEFAULT,
                                                   variables=('U3', 'U1'))
        mdb.models['Model-1'].StaticStep(maxNumInc=1000, name='Step-3'
                                         , previous='Step-2')

        mdb.models['Model-1'].StaticStep(maxNumInc=10000, name='Step-4', previous=
        'Step-3')
        ...
        mdb.models['Model-1'].sketches['__profile__'].rectangle(
            point1=(-50.0, -50.0), point2=(50.0, 50.0))
        ...
        mdb.models['Model-1'].rootAssembly.Instance(dependent=ON,
            ...
            mdb.models['Model-1'].parts['Part-2'])
        mdb.models['Model-1'].rootAssembly.translate(instanceList=(
            'Part-2-2', ), vector=(25.0, 0.0, 0.0))
        mdb.models['Model-1'].rootAssembly.translate(instanceList=(
            'Part-2-1', ), vector=(25.0, 0.0, 0.0))
        mdb.models['Model-1'].rootAssembly.translate(instanceList=(
            'Part-2-2', ), vector=(0.0, 0.0, 43))
        ...
        mdb.models['Model-1'].SurfaceToSurfaceContactStd(adjustMethod=OVERCLOSED,
            ...
            'IntProp-1', master=mdb.models['Model-1'].rootAssembly.surfaces['m_Surf-1']
            , name='Int-1', slave=mdb.models['Model-1'].rootAssembly.sets['top-surf'],
            sliding=FINITE, smooth=0.2, surfaceSmoothing=NONE, thickness=OFF, tied=OFF)
        ...
        mdb.models['Model-1'].SurfaceToSurfaceContactStd(adjustMethod=OVERCLOSED,
            clearanceRegion=None, createStepName='Step-3', datumAxis=None,
            enforcement=NODE_TO_SURFACE, initialClearance=OMIT, interactionProperty=
            'IntProp-1', master=mdb.models['Model-1'].rootAssembly.surfaces['m_Surf-2']
            , name='Int-2', slave=mdb.models['Model-1'].rootAssembly.sets['Z-face'],
            sliding=FINITE, smooth=0.2, surfaceSmoothing=NONE, thickness=OFF, tied=OFF)
        ...
        def buckling_method(method):
            ...
            if method == '2step_out_of_plane':
                ...
                mdb.models['Model-1'].DisplacementBC(amplitude=UNSET, createStepName='Step-3',
                                                     distributionType=UNIFORM, fieldName='', fixed=OFF, localCsys=None,
                                                     name=
                                                     'BC-7', region=mdb.models['Model-1'].rootAssembly.sets['Set-9'],
                                                     u1=0.0,
                                                     u2=0.0, u3=-10.0, ur1=0.0, ur2=0.0, ur3=0.0)
                ...
                a.seedPartInstance(deviationFactor=0.1, minSizeFactor=0.1,
                                   regions=(a.instances['Part-1'],), size=Square_Size / 2)
                a.generateMesh(regions=(a.instances['Part-1'],))
                mdb.models['Model-1'].parts['Part-2'].seedPart(
                    deviationFactor=0.1, minSizeFactor=0.1, size=5.0)
                ...
                mdb.Job(atTime=None, contactPrint=OFF, description='', echoPrint=OFF,
                        explicitPrecision=SINGLE, getMemoryFromAnalysis=True, historyPrint=OFF,
                        memory=90, memoryUnits=PERCENTAGE, model='Model-1', modelPrint=OFF,
                        multiprocessingMode=DEFAULT, name='a_'+str(a_tot_count)+'_b_'+str(b_tot_count), nodalOutputPrecision=FULL,
                        numCpus=1, numGPUs=0, queue=None, resultsFormat=ODB, scratch='', type=
                        ANALYSIS, userSubroutine='', waitHours=0, waitMinutes=0)

                mdb.jobs['a_'+str(a_tot_count)+'_b_'+str(b_tot_count)].writeInput()
                # mdb.jobs['a_'+str(a_tot_count)+'_b_'+str(b_tot_count)].submit(consistencyChecking=ON)
                # mdb.jobs['a_'+str(a_tot_count)+'_b_'+str(b_tot_count)].waitForCompletion()
