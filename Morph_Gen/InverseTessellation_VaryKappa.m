...
clc
% Input parameters
eps      = 0.2; % The inner circle radius
W0       = 1;
L0       = 1;
t0       = 1; t1  = 1;
N        = 6;  % Number of slices cut

% The given shape (curvature distribution)
syms s
p1 =       6395;p2 =  -2.83e+04;p3 =  5.214e+04;
p4 = -5.186e+04;p5 =  3.053e+04;p6 = -1.117e+04;
p7 =       2808;p8 =     -570.8;p9 =       89.7;
% f(x) = p1*x^8 + p2*x^7 + p3*x^6 + p4*x^5 + p5*x^4 + p6*x^3 + p7*x^2 + p8*x + p9;
theta    =       p1*s.^8 + p2*s.^7 + p3*s.^6 + p4*s.^5 + p5*s.^4 + p6*s.^3 + p7*s.^2 + p8*s + p9;
dtheta   =  diff(p1*s.^8 + p2*s.^7 + p3*s.^6 + p4*s.^5 + p5*s.^4 + p6*s.^3 + p7*s.^2 + p8*s + p9,s);
sintheta = @(s) sin((p1*s.^8 + p2*s.^7 + p3*s.^6 + p4*s.^5 + p5*s.^4 + p6*s.^3 + p7*s.^2 + p8*s + p9)/180*pi);
costheta = @(s) cos((p1*s.^8 + p2*s.^7 + p3*s.^6 + p4*s.^5 + p5*s.^4 + p6*s.^3 + p7*s.^2 + p8*s + p9)/180*pi);

% Looking for the max point
theta1    = @(s)      p1*s.^8 + p2*s.^7 + p3*s.^6 + p4*s.^5 + p5*s.^4 + p6*s.^3 + p7*s.^2 + p8*s + p9;
...
str2 = append('w_newshape_varkappa');
str2 = strrep(str2,'.','');
...
str1 = append('vf_newshape_varkappa_3mat');
str1 = strrep(str1,'.','');
